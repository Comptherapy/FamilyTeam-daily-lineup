...
# ---------------------------------------------------------------------------
# HEADER — inside a scoreboard-style panel
# ---------------------------------------------------------------------------
with st.container(key="scoreboard_panel"):
    st.markdown('<div class="stripe-band stripe-top"></div>', unsafe_allow_html=True)

    theme.title_with_icon("The Daily Lineup")
    ...
    st.markdown('<div class="stripe-band stripe-bottom"></div>', unsafe_allow_html=True)
...
with tab_am:
    theme.section_header("STARTING LINEUP", "tag-lineup", "Have to")
    ...
    theme.section_header("BONUS ROUND", "tag-bonus", "Want to, if time's left")
...
with tab_after:
    theme.section_header("HALFTIME LINEUP", "tag-lineup", "Have to, at the clinic")
...
with tab_pm:
    theme.section_header("CLOSING LINEUP", "tag-lineup", "Have to, after practice/game")
    ...
    theme.section_header("FRIDAY ONLY", "tag-bonus", "If the week was a success")
    mon_thu_done = all(
...
with tab_coach:
    with st.container(key="whiteboard"):
        theme.section_header("COACH'S LIST", "tag-lineup", "Your own routine")
