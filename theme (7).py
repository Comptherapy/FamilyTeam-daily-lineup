...
def title_with_icon(title_text):
    """Renders the main app title with a basketball + jersey icon beside it (light text, for the scoreboard panel)."""
    st.markdown(
...
def referee_icon(size=40, stroke="#1A1A1A"):
    """Inline SVG referee jersey (black/white stripes) with a whistle on a lanyard."""
    return (
        f'<svg width="{size}" height="{size}" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">'
        f'<defs><clipPath id="refShirtClip"><path d="M14 4 L7 9 L11 17 L14 14.5 V44 H34 V14.5 L37 17 L41 9 L34 4 '
        f'Q30 8.5 24 8.5 Q18 8.5 14 4 Z"/></clipPath></defs>'
        f'<path d="M14 4 L7 9 L11 17 L14 14.5 V44 H34 V14.5 L37 17 L41 9 L34 4 '
        f'Q30 8.5 24 8.5 Q18 8.5 14 4 Z" fill="{WHITE}" stroke="{stroke}" stroke-width="2.5" stroke-linejoin="round"/>'
        f'<g clip-path="url(#refShirtClip)">'
        f'<rect x="8" y="0" width="5" height="48" fill="{stroke}"/>'
        f'<rect x="18" y="0" width="5" height="48" fill="{stroke}"/>'
        f'<rect x="28" y="0" width="5" height="48" fill="{stroke}"/>'
        f'<rect x="38" y="0" width="5" height="48" fill="{stroke}"/>'
        f'</g>'
        f'<line x1="24" y1="17" x2="24" y2="30" stroke="{stroke}" stroke-width="1.5"/>'
        f'<circle cx="24" cy="34" r="5" fill="{MUSTARD}" stroke="{stroke}" stroke-width="1.5"/>'
        f'<circle cx="24" cy="34" r="1.5" fill="{stroke}"/>'
        f'</svg>'
    )


def trophy_icon(size=40, stroke="#1A1A1A"):
    """Inline SVG trophy — used to badge Game pages so they read differently from Practice pages."""
    return (
        f'<svg width="{size}" height="{size}" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">'
        f'<path d="M14 6 H34 V16 Q34 26 24 26 Q14 26 14 16 Z" fill="{MUSTARD}" stroke="{stroke}" stroke-width="2"/>'
        f'<path d="M14 8 H6 Q6 18 14 18" fill="none" stroke="{stroke}" stroke-width="2"/>'
        f'<path d="M34 8 H42 Q42 18 34 18" fill="none" stroke="{stroke}" stroke-width="2"/>'
        f'<rect x="21" y="26" width="6" height="8" fill="{MUSTARD}" stroke="{stroke}" stroke-width="2"/>'
        f'<rect x="15" y="36" width="18" height="4" rx="1" fill="{MUSTARD}" stroke="{stroke}" stroke-width="2"/>'
        f'</svg>'
    )


def game_icon_cluster(sport_icon_html, trophy_size=48):
    """Badges a small trophy onto the bottom-right corner of a sport icon, for Game pages."""
    trophy = trophy_icon(trophy_size)
    return (
        f'<div style="position:relative; display:inline-block;">'
        f'{sport_icon_html}'
        f'<div style="position:absolute; bottom:-14px; right:-14px;">{trophy}</div>'
        f'</div>'
    )
